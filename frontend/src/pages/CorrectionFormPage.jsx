...
export default function CorrectionFormPage() {
  ...
  const [acctSearch, setAcctSearch] = useState({});  // { rowIdx: 'q' }
  const [acctOptions, setAcctOptions] = useState({}); // { rowIdx: [accts] }
  ...
  const lookup = async () => {
    ...
    try {
      ...
      if (lookupMode === 'journal') {
        ...
        raw = data;
        header = { journal_id: data[0].journal_id, journal_entry_id: data[0].journal_entry_id, order_number: data[0].order_number, pr_finance_id: data[0].pr_finance_id, journal_description: data[0].journal_description };
      } else {
        const { data } = await api.get(`/journal-entries/${lookupId}`);
        // Auto-fetch sibling entries from same journal
        const { data: all } = await api.get(`/journal/${data.journal_id}/entries`);
        raw = all;
        header = { journal_id: data.journal_id, journal_entry_id: data.journal_entry_id, order_number: data.order_number, pr_finance_id: data.pr_finance_id, journal_description: data.journal_description };
      }
  ...
  const canProceed3 = entries.length >= 2 && origBal.ok && corrBal.ok && reason.trim().length >= 10;
  ...
  const submit = async (action) => {
    ...
    try {
      const { data } = await api.post('/corrections', {
        ...
        source_journal_entry_id: journalInfo.journal_entry_id,
        entries,
      });
      if (files.length > 0) {
      ...
      if (action === 'submit') {
        await api.post(`/corrections/${data.id}/submit`);
      }
